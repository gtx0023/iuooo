<template>
  <div class="salesTable">
    <el-table :data="tableData" stripe height="424" style="width: 100%">
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.status === 1" class="saleColor"
            >$ {{ scope.row.price }}</span
          >
          <span v-if="scope.row.status === 2" class="taxColor"
            >$ {{ scope.row.price }}</span
          >
          <span v-if="scope.row.status === 3" class="extenedColor"
            >$ {{ scope.row.price }}</span
          >
          <span v-if="scope.row.status0 === 4" class="likeColor"
            >$ {{ scope.row.price }}</span
          >
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="salesStatus" prop="status" label="STATUS">
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.status === 3" class="extenedBgcolor"
            >EXTENDED</span
          >
